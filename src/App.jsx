          ...
          return (
            <div key={row.id} style={{background:"#ffffff",border:`1px solid ${isDone?"#f3c0c8":isScheduled?"#16a34a44":urg.bg}`,borderRadius:9,overflow:"hidden",opacity:isDone?0.6:1,transition:"all 0.2s"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",cursor:"pointer",flexWrap:"wrap"}} onClick={()=>setExpandedId(expanded?null:row.id)}>
                ...
                {/* Customer + inline comment */}
                <div style={{flex:1,minWidth:80,overflow:"hidden"}}>
                  <div style={{fontSize:11,color:isDone?"#e8b4bc":"#a07880",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row.customer}</div>
                  {(row.notes||row.comment)&&!isDone&&(
                    <div style={{fontSize:10,color:"#f59e0b",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:1}}>
